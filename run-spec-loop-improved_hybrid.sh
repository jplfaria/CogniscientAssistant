...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
    ...
    cat specs-prompt_hybrid.md | claude -p --dangerously-skip-permissions 2>&1 | tee "$TEMP_OUTPUT"
    
    echo -e "\n${MAGENTA}--- End of Claude's response ---${NC}"
    
    # Check if all tasks are complete by looking for unchecked items
    UNCHECKED_COUNT=$(grep -E "^\- \[ \]" SPECS_PLAN_hybrid.md | wc -l | tr -d ' ')
    ...
    # Show ALL progress on SPECS_PLAN_hybrid.md (not just first 10)
    echo -e "\n${YELLOW}Complete progress on SPECS_PLAN_hybrid.md:${NC}"
    echo -e "${CYAN}Completed specs:${NC}"
    grep -E "^\- \[x\]" SPECS_PLAN_hybrid.md | nl -nln -w2 -s'. '
    
    echo -e "\n${CYAN}Remaining specs ($UNCHECKED_COUNT):${NC}"
    grep -E "^\- \[ \]" SPECS_PLAN_hybrid.md | head -5
    ...
    ls -t specs_hybrid/*.md 2>/dev/null | head -3
...
echo -e "  ${GREEN}✓${NC} Specs created: $(ls specs_hybrid/*.md 2>/dev/null | wc -l | tr -d ' ')"
echo -e "  ${GREEN}✓${NC} Remaining tasks: $UNCHECKED_COUNT"
echo -e "\n${YELLOW}To review:${NC}"
echo -e "  ${CYAN}All changes:${NC} git status"
echo -e "  ${CYAN}All specs:${NC} ls -la specs_hybrid/"
echo -e "  ${CYAN}Full plan:${NC} cat SPECS_PLAN_hybrid.md"