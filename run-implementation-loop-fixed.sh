...
# run-implementation-loop-fixed.sh
# Fixed version of the implementation loop that handles Claude invocation properly
...
echo -e "${GREEN}=== AI Co-Scientist Implementation Loop (Fixed) ===${NC}"
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
    ...
    # Use a here-doc to avoid pipe issues
    claude -p --dangerously-skip-permissions "$PROMPT_CONTENT" 2>&1 | tee "$TEMP_OUTPUT"
    
    echo -e "\n${MAGENTA}--- End of Claude's implementation ---${NC}"
    
    # Check if implementation is complete
    if grep -q "Implementation complete\|All components implemented\|No more implementation tasks\|IMPLEMENTATION_COMPLETE" "$TEMP_OUTPUT" 2>/dev/null; then
